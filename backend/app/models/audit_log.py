...
class AuditLog(Base):
    ...
    user_id = Column(Integer, ForeignKey("users.id", ondelete="SET NULL"), nullable=True, comment="操作用户ID")
    username = Column(String(50), nullable=True, comment="操作用户名（冗余字段）")
    action = Column(String(50), nullable=False, comment="操作类型")
    resource_type = Column(String(50), nullable=True, comment="资源类型（project/element/step等）")
    ...
    __table_args__ = (
        ...
        # 复合索引 - 支持常见查询模式
        Index('ix_audit_log_project_time', 'project_id', 'created_at'),  # 按项目+时间查询
        Index('ix_audit_log_user_time', 'user_id', 'created_at'),        # 按用户+时间查询
        Index('ix_audit_log_action_time', 'action', 'created_at'),      # 按操作类型+时间查询

        # 支持资源查询的复合索引
        Index('ix_audit_log_resource', 'resource_type', 'resource_id'),  # 按资源查询
    )
