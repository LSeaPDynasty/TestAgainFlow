"""add error status to run_result_enum

Revision ID: 007_add_error_to_run_result
Revises: 006_add_test_plan_run_type
...
revision = '007_add_error_to_run_result'
down_revision = '006_add_test_plan_run_type'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('type', sa.Enum('testcase', 'suite', 'test_plan', name='run_type_enum'), nullable=False, comment='Run type'),
        ...
        sa.Column('result', sa.Enum('pending', 'running', 'pass', 'fail', 'cancelled', 'timeout', 'error', name='run_result_enum_v2'), nullable=False, default='pending', comment='Execution result'),
    ...
    # Step 2: Copy data from old table to new table, converting 'error' to 'fail'
    op.execute("""
        ...
        (id, task_id, type, target_id, target_name, result, returncode, duration,
         ...
         log_path, project_id, test_plan_id, total_count, success_count, failed_count, skipped_count,
         started_at, finished_at, created_at, updated_at)
        SELECT id, task_id, type, target_id, target_name,
               CASE WHEN result = 'error' THEN 'fail' ELSE result END,
               returncode, duration,
               profile_id, profile_name, device_serial, device_name, has_screenshots,
               log_path, project_id, test_plan_id, total_count, success_count, failed_count, skipped_count,
               started_at, finished_at, created_at, updated_at
...
def downgrade():
    """Remove 'error' status from run_result_enum"""

    # Revert to original enum (remove 'error')
    op.create_table(
        'run_history_old',
        sa.Column('id', sa.Integer(), primary_key=True, autoincrement=True),
        sa.Column('task_id', sa.String(length=100), nullable=False, unique=True),
        sa.Column('type', sa.Enum('testcase', 'suite', 'test_plan', name='run_type_enum'), nullable=False),
