...
运行方式：
    # 迁移到项目1931
    python scripts/migrate_to_project.py

    # 迁移到指定项目ID
    python scripts/migrate_to_project.py --project-id 123
"""
import sys
import os
import argparse
...
def migrate_to_project(project_id: int = 1931):
    """将所有现有数据迁移到指定项目"""
    print(f"开始将现有数据迁移到项目 {project_id}...")
    print("=" * 60)
        ...
        with engine.connect() as conn:
            ...
            if not project:
                ...
                if projects:
                    ...
                    print(f"\n使用方法: python scripts/migrate_to_project.py --project-id <项目ID>")
                return
            ...
            for table_name in tables_to_migrate:
                ...
                if 'project_id' not in columns:
                    print(f"⚠️ {table_name} 表没有 project_id 列，跳过")
                    continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='将数据迁移到指定项目')
    parser.add_argument('--project-id', type=int, default=1931,
                       help='目标项目ID（默认: 1931）')
    args = parser.parse_args()

    migrate_to_project(args.project_id)
